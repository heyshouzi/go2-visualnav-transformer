...
def train_3d_nomad(
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            batch_obs_images = torch.cat(batch_obs_images, dim=1).to(device)
            batch_goal_images = transform(goal_image).to(device)
            batch_obs_lidars = obs_lidar.to(device)
            batch_goal_lidars = goal_lidar.to(device)
            action_mask = action_mask.to(device)
            ...
            # Generate random goal mask
            goal_mask = (torch.rand((B,)) < goal_mask_prob).long().to(device)
            lidar_mask = (torch.rand((B,)) < lidar_mask_prob).long().to(device)
            obsgoal_cond = model("vision_lidar_encoder", 
            ...
            if i % print_log_freq == 0:
                losses = _compute_losses_3d_nomad(
                            ema_model.averaged_model,
                            noise_scheduler,
                            batch_obs_images,
                            batch_goal_images,
            ...
            if image_log_freq != 0 and i % image_log_freq == 0:
                visualize_3d_nomad_diffusion_action_distribution(
                    ema_model.averaged_model,
                    noise_scheduler,
                    batch_obs_images,
                    batch_goal_images,
...
def evaluate_3d_nomad(
    ...
    with tqdm.tqdm(
        ...
        for i, data in enumerate(tepoch):
            ...
            rand_mask_cond = ema_model("vision_lidar_encoder", 
                                        obs_img=batch_obs_images, 
                                        goal_img=batch_goal_images, 
                                        obs_lidar=batch_obs_lidars, 
                                        goal_lidar=batch_goal_lidars,
            ...
            goal_mask_cond = ema_model("vision_lidar_encoder", 
                                        obs_img=batch_obs_images, 
                                        goal_img=batch_goal_images, 
                                        obs_lidar=batch_obs_lidars, 
                                        goal_lidar=batch_goal_lidars,
...
def model_3d_nomad_output(
    model: nn.Module,
    noise_scheduler: DDPMScheduler,
    batch_obs_images: torch.Tensor,
    batch_goal_images: torch.Tensor,
    batch_obs_lidars:torch.Tensor,
    batch_goal_lidars: torch.Tensor,
    pred_horizon: int,
    action_dim: int,
    num_samples: int,
    device: torch.device,
    ...
    obs_cond = model("vision_lidar_encoder", 
                        obs_img=batch_obs_images, 
                        goal_img=batch_goal_images, 
                        obs_lidar=batch_obs_lidars, 
                        goal_lidar=batch_goal_lidars,
...
def visualize_3d_nomad_diffusion_action_distribution(
    ema_model: nn.Module,
    noise_scheduler: DDPMScheduler,
    batch_obs_images: torch.Tensor,
    batch_goal_images: torch.Tensor,
    ...
    num_images_log = min(num_images_log, 
    ...
    batch_obs_lidars = batch_obs_lidars[:num_images_log]
    batch_goal_lidars = batch_goal_lidars[:num_images_log]
    ...
    batch_obs_lidars_list = torch.split(batch_obs_lidars, max_batch_size, dim=0)
    batch_goal_lidars_list = torch.split(batch_goal_lidars, max_batch_size, dim=0)
    uc_actions_list = []
    gc_actions_list = []
    gc_distances_list = []
