...
from vint_train.training.train_utils import train_3d_nomad, evaluate_3d_nomad
import torch
...
def train_eval_loop_3d_nomad(
    ...
    for epoch in range(current_epoch, current_epoch + epochs):
        if train_model:
            print(
            f"Start 3D NoMaD Training Epoch {epoch}/{current_epoch + epochs - 1}"
            )
            train_3d_nomad(
                model=model,
                ema_model=ema_model,
        ...
        if (epoch + 1) % eval_freq == 0: 
            for dataset_type in test_dataloaders:
                print(
                    f"Start {dataset_type} 3D NoMaD Testing Epoch {epoch}/{current_epoch + epochs - 1}"
                )
                loader = test_dataloaders[dataset_type]
                evaluate_3d_nomad(
                    eval_type=dataset_type,
