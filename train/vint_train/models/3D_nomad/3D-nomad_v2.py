...
class NoMaD_3D_Nomad(nn.Module):
    def __init__(
        self,
        context_size: int = 5,
        ...
        lidar_encoder: Optional[nn.Module] = None,  # LiDAR网络（例如LiDARNet）
    ) -> None:
        """
        NoMaD 3D-Nomad Encoder class
        """
        super().__init__()
        ...
        # LiDAR encoder (can be a custom network like LiDARNet)
        self.lidar_encoder = lidar_encoder

        # Compression layers if necessary
        self.compress_obs_enc = nn.Linear(self.num_obs_features, self.obs_encoding_size) if self.num_obs_features != self.obs_encoding_size else nn.Identity()
        self.compress_goal_enc = nn.Linear(self.num_goal_features, self.goal_encoding_size) if self.num_goal_features != self.goal_encoding_size else nn.Identity()
    ...
    def forward(
        ...
        """
        Forward pass with optional lidar input controlled by mask.
        """
        ...
        # Get the goal mask (if provided)
        if input_goal_mask is not None:
            goal_mask = input_goal_mask.to(device)

        # Get goal encoding
        obsgoal_img = torch.cat([obs_img[:, 3 * self.context_size:, :, :], goal_img], dim=1)  # Concatenate observation and goal
        obsgoal_encoding = self.goal_encoder.extract_features(obsgoal_img)  # Encoding the image
        ...
        obs_img = torch.split(obs_img, 3, dim=1)  # Split observation into frames
        ...
        # LiDAR processing if needed (only if lidar_mask is 1)
        if lidar_data is not None and (lidar_mask is None or lidar_mask == 1):
            lidar_encoding = self.lidar_encoder(lidar_data)  # Using the LiDAR encoder (e.g., LiDARNet)
            lidar_encoding = lidar_encoding.unsqueeze(1)
            lidar_encoding = lidar_encoding.reshape((self.context_size + 1, -1, self.obs_encoding_size))
            lidar_encoding = torch.transpose(lidar_encoding, 0, 1)
            obs_encoding = torch.cat((obs_encoding, lidar_encoding), dim=1) 
